...
use phoenix_api_policy_decrypt::control_plane;
...
pub(crate) struct DecryptEngine {
    pub(crate) node: DataPathNode,

    pub(crate) indicator: Indicator,

    pub(crate) config: DecryptConfig,
...
impl Engine for DecryptEngine {
    fn activate<'a>(self: Pin<&'a mut Self>) -> BoxFuture<'a, EngineResult> {
        Box::pin(async move { self.get_mut().mainloop().await })
    }

    fn description(self: Pin<&Self>) -> String {
        "DecryptEngine".to_owned()
    }
...
impl_vertex_for_engine!(DecryptEngine, node);

impl Decompose for DecryptEngine {
