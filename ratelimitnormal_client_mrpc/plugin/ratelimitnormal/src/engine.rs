...
use phoenix_api_policy_ratelimitnormal::control_plane;
...
pub(crate) struct RatelimitnormalEngine {
    ...
    pub(crate) config: RatelimitnormalConfig,
...
impl Engine for RatelimitnormalEngine {
    fn activate<'a>(self: Pin<&'a mut Self>) -> BoxFuture<'a, EngineResult> {
        Box::pin(async move { self.get_mut().mainloop().await })
    }

    fn description(self: Pin<&Self>) -> String {
        "RatelimitnormalEngine".to_owned()
    }
    ...
    fn handle_request(&mut self, request: Vec<u8>, _cred: UCred) -> Result<()> {
        ...
        match request {
            control_plane::Request::NewConfig(requests_per_sec, bucket_size) => {
                self.config = RatelimitnormalConfig {
                    requests_per_sec,
                    bucket_size,
                };
            }
...
impl_vertex_for_engine!(RatelimitnormalEngine, node);

impl Decompose for RatelimitnormalEngine {
...
impl RatelimitnormalEngine {
    pub(crate) fn restore(
        ...
        let config = *local
            .remove("config")
            .unwrap()
            .downcast::<RatelimitnormalConfig>()
        ...
        let engine = RatelimitnormalEngine {
            node,
            ...
            Ok(msg) => {
                match msg {
                    EngineTxMessage::RpcMessage(req) => {
                        self.num_tokens = (self.config.bucket_size as f64).min(
                            self.num_tokens
